...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, RefreshControl, KeyboardAvoidingView, Platform, Modal, Pressable } from "react-native";
...
export default function AdminTickets() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bgApp }} edges={["top"]}>
      ...
      >
        {tickets.length === 0 ? (
          <View style={styles.empty}><Ionicons name="chatbox-ellipses-outline" size={44} color={colors.textDisabled} /><Text style={styles.emptyTxt}>No tickets yet.</Text></View>
        ) : tickets.map(t => (
          <TouchableOpacity key={t.id} onPress={() => { setEditing(t); setReply(t.reply || ""); setStatus((t.status as any) || "in_progress"); }} style={styles.card} testID={`ticket-${t.id}`}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <Text style={styles.subject} numberOfLines={1}>{t.subject}</Text>
              <View style={[styles.badge, styles[`b_${t.status}` as keyof typeof styles] as any]}>
            ...
            <Pressable style={styles.sheet} onPress={e => e.stopPropagation()}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
                {(["open", "in_progress", "resolved", "closed"] as const).map(s => (
                  <TouchableOpacity key={s} onPress={() => setStatus(s)} style={[styles.chip, status === s && styles.chipActive]} testID={`ticket-status-${s}`}>
...
const styles = StyleSheet.create({
  ...
  b_open: { backgroundColor: colors.warningLight },
  b_in_progress: { backgroundColor: colors.infoLight },
  b_resolved: { backgroundColor: colors.successLight },
  b_closed: { backgroundColor: colors.bgMuted },
