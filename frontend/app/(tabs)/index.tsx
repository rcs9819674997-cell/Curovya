...
const QUICK_KEYS = [
  { id: "book", icon: "calendar" as const, key: "book_doctor", color: "#DC143C", route: "/find-doctor" },
  { id: "lab", icon: "flask" as const, key: "lab_tests", color: "#0EA5E9", route: "/labs" },
  { id: "reminders", icon: "alarm" as const, key: "medicine_reminders", color: "#F59E0B", route: "/reminders" },
  { id: "sym", icon: "sparkles" as const, key: "symptom_checker", color: "#8B5CF6", route: "/symptom-checker" },
];
...
export default function Home() {
  ...
  useEffect(() => {
    const loop = Animated.loop(
      Animated.sequence([
        Animated.timing(livePulse, { toValue: 1.7, duration: 650, useNativeDriver: true }),
  ...
  const patientsAhead = next ? Math.max(0, next.token_number - next.current_serving - 1) : 0;
  const isBeingCalled = next ? next.current_serving >= next.token_number : false;
  const progressPct = next
    ? Math.min(100, Math.max(4, (next.current_serving / Math.max(next.token_number, 1)) * 100))
    : 0;
  ...
  const queueStatusColor = isBeingCalled || patientsAhead === 0 ? SUCCESS : colors.text;
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bgApp }} edges={["top"]}>
      ...
      >
        ...
        <View style={{ padding: spacing.lg }}>
          {error && !loading && (
            <TouchableOpacity style={styles.errorBanner} onPress={load} testID="home-error-retry">
              <Ionicons name="alert-circle" size={18} color={colors.primary} />
              <Text style={styles.errorText}>Couldn&apos;t refresh your data. Tap to retry.</Text>
        ...
        {t(q.key)}
          ...
          {loading ? (
            <Card style={{ padding: spacing.lg }}>
              ...
              <Animated.View style={[styles.skeletonLine, { width: "100%", height: 48, marginTop: 20, opacity: pulse }]} />
            </Card>
          ) : next ? (
            <Card
              testID={`upcoming-appointment-${next.id}`}
              onPress={() => router.push({ pathname: "/ticket/[id]", params: { id: next.id } })}
            ...
            >
              ...
              <View style={styles.appRow}>
                <Image source={{ uri: next.doctor_photo_url }} style={styles.avatar} />
              ...
              <View style={styles.queueBlock}>
                ...
                <Text style={[styles.queueStatus, { color: queueStatusColor }]}>{queueStatusText}</Text>
              </View>
            ...
            <Card style={{ alignItems: "center", paddingVertical: spacing.xl }}>
              ...
              <TouchableOpacity onPress={() => router.push("/find-doctor")} style={{ marginTop: 8 }}>
                <Text style={{ color: colors.primary, fontWeight: "700" }}>Book your first appointment →</Text>
              </TouchableOpacity>
            </Card>
          )}
          ...
          <View style={{ flexDirection: "row", gap: 12 }}>
            <Card style={[styles.statCard, { backgroundColor: "#FFF7ED" }]}>
              ...
              <Text style={styles.summaryVal}>{loading ? "–" : rxs.length}</Text>
            ...
            <Card style={[styles.statCard, { backgroundColor: colors.infoLight }]}>
              ...
              <Text style={styles.summaryVal}>{loading ? "–" : appts.length}</Text>
          ...
          <View style={styles.rowBetween}>
            ...
            {!loading && rxs.length > 0 && (
              <TouchableOpacity onPress={() => router.push("/(tabs)/prescriptions" as any)}>
                <Text style={styles.link}>See all</Text>
              </TouchableOpacity>
            )}
          </View>
          ...
          {loading ? (
            <Card style={{ marginBottom: spacing.md }}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <Animated.View style={[styles.skeletonRxIcon, { opacity: pulse }]} />
                <View style={{ flex: 1, marginLeft: 12 }}>
                  <Animated.View style={[styles.skeletonLine, { width: "70%", opacity: pulse }]} />
                  <Animated.View style={[styles.skeletonLine, { width: "45%", marginTop: 6, opacity: pulse }]} />
                </View>
              </View>
            </Card>
          ) : rxs.length === 0 ? (
            <Card style={{ alignItems: "center", paddingVertical: spacing.lg }}>
              ...
              <Text style={{ color: colors.textSecondary, marginTop: 8 }}>No prescriptions yet</Text>
            </Card>
          ) : (
            rxs.slice(0, 3).map((r) => (
              <Card
                ...
                onPress={() => router.push({ pathname: "/prescriptions/[id]", params: { id: r.id } })}
                style={{ marginBottom: spacing.md }}
              >
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  <View style={styles.rxIcon}>
                    <Ionicons name="document-text" size={19} color={colors.primary} />
                  </View>
                  <View style={{ flex: 1, marginLeft: 12 }}>
          ...
          >
            ...
            <View style={{ flex: 1, marginLeft: 12 }}>
              ...
              <Text style={styles.emSub}>Call ambulance · Nearby hospitals · Blood banks</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  eyebrow: { color: "rgba(255,255,255,0.8)", fontSize: 13, fontWeight: "600", marginBottom: 2 },
  hello: { color: "#fff", fontSize: 24, fontWeight: "800", letterSpacing: -0.3 },
  bell: {
    width: 42,
    height: 42,
    borderRadius: 21,
  ...
  dot: { position: "absolute", top: 9, right: 9, width: 8, height: 8, borderRadius: 4, backgroundColor: "#FDE047" },
  search: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: "#fff",
  ...
  section: { fontSize: 16, fontWeight: "700", color: colors.text, letterSpacing: -0.2, marginTop: spacing.lg, marginBottom: spacing.md },
...
quickTxt: {
  ...
  statusStrip: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: spacing.lg, paddingTop: spacing.md },
  statusPill: { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "#ECFDF5", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999 },
  statusPillDot: { width: 6, height: 6, borderRadius: 3, backgroundColor: SUCCESS },
  ...
  divider: { height: 1, backgroundColor: HAIRLINE, marginHorizontal: spacing.lg },
  ...
  liveBadgeText: { fontSize: 11, fontWeight: "700", color: SUCCESS, letterSpacing: 0.5 },

  tokenRow: { flexDirection: "row", alignItems: "center", marginTop: spacing.md },
  tokenDividerV: { width: 1, height: 32, backgroundColor: HAIRLINE, marginHorizontal: spacing.md },
  tokenLabel: { fontSize: 11, color: colors.textSecondary, fontWeight: "600", textTransform: "uppercase", letterSpacing: 0.3 },
  tokenNum: { fontSize: 24, fontWeight: "800", color: colors.primary, marginTop: 2 },
  serving: { fontSize: 24, fontWeight: "800", color: colors.text, marginTop: 2 },

  progressTrack: { height: 6, borderRadius: 3, backgroundColor: "#F1F1F3", marginTop: spacing.md, overflow: "hidden" },
  progressFill: { height: 6, borderRadius: 3, backgroundColor: colors.primary },
  queueStatus: { fontSize: 13, fontWeight: "600", marginTop: 8 },

  viewTicketRow: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: colors.primary, paddingVertical: 14 },
  viewTicketText: { color: "#fff", fontWeight: "700", fontSize: 14 },

  emptyIconWrap: { width: 48, height: 48, borderRadius: 24, backgroundColor: colors.primaryLight, alignItems: "center", justifyContent: "center" },
  ...
  summaryVal: { fontSize: 24, fontWeight: "800", color: colors.text, marginTop: 10 },
  summaryLabel: { fontSize: 12, color: colors.textSecondary, marginTop: 2, fontWeight: "600" },

  rxIcon: { width: 44, height: 44, borderRadius: 12, backgroundColor: colors.primaryLight, alignItems: "center", justifyContent: "center" },
  ...
  emDecorCircle: { position: "absolute", top: -30, right: -20, width: 100, height: 100, borderRadius: 50, backgroundColor: "rgba(255,255,255,0.08)" },
  emIconWrap: { width: 40, height: 40, borderRadius: 20, backgroundColor: "rgba(255,255,255,0.18)", alignItems: "center", justifyContent: "center" },
  emTitle: { color: "#fff", fontSize: 16, fontWeight: "700" },
  emSub: { color: "rgba(255,255,255,0.85)", fontSize: 12, marginTop: 2 },
  emChevronWrap: { width: 30, height: 30, borderRadius: 15, backgroundColor: "rgba(255,255,255,0.18)", alignItems: "center", justifyContent: "center" },

  skeletonAvatar: { width: 56, height: 56, borderRadius: 28, backgroundColor: SKELETON },
  skeletonRxIcon: { width: 44, height: 44, borderRadius: 12, backgroundColor: SKELETON },
