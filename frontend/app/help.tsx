...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, KeyboardAvoidingView, Platform, RefreshControl } from "react-native";
...
export default function HelpSupport() {
  ...
  const [category, setCategory] = useState<"appointment" | "payment" | "technical" | "prescription" | "other">("other");
  ...
  const categories: { key: typeof category; label: string; icon: keyof typeof Ionicons.glyphMap }[] = [
              ...
              {faqs.map((f, i) => (
                ...
                >
                  <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 10 }}>
                    ...
                    <Ionicons name={openFaq === i ? "chevron-up" : "chevron-down"} size={18} color={colors.textDisabled} />
                  </View>
                </TouchableOpacity>
              ))}
          ...
          {tab === "tickets" ? (
            <View>
              {tickets.length === 0 ? (
                ...
                tickets.map(t => (
                  <View key={t.id} style={styles.ticketCard} testID={`ticket-card-${t.id}`}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                      <Text style={styles.tSubject} numberOfLines={1}>{t.subject}</Text>
                      <View style={[styles.status, styles[`status_${t.status}` as keyof typeof styles] as any]}>
...
const styles = StyleSheet.create({
  ...
  status_open: { backgroundColor: colors.warningLight },
  status_in_progress: { backgroundColor: colors.infoLight },
  status_resolved: { backgroundColor: colors.successLight },
  status_closed: { backgroundColor: colors.bgMuted },
