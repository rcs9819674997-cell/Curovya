...
const DoseRow = React.memo(function DoseRow({
  ...
  d: Dose;
  index: number;
  onLog: (dose: Dose, status: "taken" | "skipped") => void;
}) {
  const isTaken = d.status === "taken";
  const isSkipped = d.status === "skipped";

  return (
    <Card style={{ marginBottom: spacing.md, padding: 0, overflow: "hidden" }} testID={`dose-${index}`}>
      ...
      <View style={styles.actions}>
        {isTaken ? (
          ...
          <>
            <TouchableOpacity onPress={() => onLog(d, "skipped")} style={styles.skipBtn} testID={`skip-dose-${index}`}>
              <Ionicons name="close" size={14} color={colors.textSecondary} />
              <Text style={{ color: colors.textSecondary, fontWeight: "700", marginLeft: 4, fontSize: 12 }}>Skip</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => onLog(d, "taken")} style={styles.takeBtn} testID={`take-dose-${index}`}>
...
// ── Extracted reminder row component ─────────────────────────────────────
const ReminderRow = React.memo(function ReminderRow({
  ...
  return (
    <Card style={{ marginBottom: spacing.md }} testID={`reminder-${r.id}`}>
      <View style={{ flexDirection: "row", alignItems: "center" }}>
        ...
        <TouchableOpacity onPress={() => onRemove(r)} style={styles.trashBtn} testID={`delete-reminder-${r.id}`}>
...
export default function RemindersScreen() {
  ...
  const logDose = useCallback(async (dose: Dose, status: "taken" | "skipped") => {
    try {
  ...
  const removeReminder = useCallback((r: Reminder) => {
    Alert.alert(
      "Delete Reminder",
  ...
  const renderDose = useCallback(({ item, index }: { item: Dose; index: number }) => (
    <DoseRow d={item} index={index} onLog={logDose} />
  ), [logDose]);

  const renderReminder = useCallback(({ item }: { item: Reminder }) => (
    <ReminderRow r={item} onRemove={removeReminder} />
  ), [removeReminder]);

  const renderDosesEmpty = useCallback(() => (
  ...
  // Combine doses + reminders into a single FlatList for best scroll perf
  type ListItem = { type: "header-adherence" } | { type: "header-schedule" } | { type: "dose"; dose: Dose; index: number } | { type: "doses-empty" } | { type: "header-meds" } | { type: "reminder"; reminder: Reminder };

  const listData: ListItem[] = [];

  // Adherence card
  if (summary && summary.counts.total > 0) {
    listData.push({ type: "header-adherence" });
  }

  // Schedule header
  listData.push({ type: "header-schedule" });

  // Doses or empty
  if (loading || !summary || summary.doses.length === 0) {
    if (!loading) listData.push({ type: "doses-empty" });
  } else {
    summary.doses.forEach((d, i) => listData.push({ type: "dose", dose: d, index: i }));
  }

  // Active reminders
  if (activeReminders.length > 0) {
    listData.push({ type: "header-meds" });
    activeReminders.forEach(r => listData.push({ type: "reminder", reminder: r }));
  }

  const renderListItem = useCallback(({ item }: { item: ListItem }) => {
    switch (item.type) {
      case "header-adherence":
        return (
      ...
      case "header-schedule":
        return <Text style={styles.h}>Today&apos;s Schedule</Text>;
      case "doses-empty":
        return renderDosesEmpty();
      case "dose":
        return <DoseRow d={item.dose} index={item.index} onLog={logDose} />;
      case "header-meds":
        return <Text style={styles.h}>All Medications ({activeReminders.length})</Text>;
      case "reminder":
        return <ReminderRow r={item.reminder} onRemove={removeReminder} />;
      default:
        return null;
    }
  }, [summary, logDose, removeReminder, activeReminders.length, renderDosesEmpty]);

  const keyExtractor = useCallback((_: ListItem, index: number) => `item-${index}`, []);
