...
export default function Login() {
  ...
  const { login } = useAuth();
  const [email, setEmail] = useState(__DEV__ ? "patient@hamrodoctor.np" : "");
  const [password, setPassword] = useState(__DEV__ ? "Patient@123" : "");
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (u.role && u.role !== "patient") {
        setErr("This app is for patients. Please use the appropriate app for your account.");
        return;
      }
      router.replace("/(tabs)");
    } catch (e: any) {
      setErr(e?.detail || "Login failed. Please try again.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }} edges={["top", "bottom"]}>
      ...
      >
        <ScrollView contentContainerStyle={styles.wrap} keyboardShouldPersistTaps="handled">
          ...
          <Text style={styles.sub}>Sign in to continue to Curovya</Text>
          ...
          {__DEV__ ? (
            <View style={styles.demoBox}>
              ...
              <View style={{ flex: 1, marginLeft: 6 }}>
                <Text style={styles.demoTxt}>Patient: patient@hamrodoctor.np / Patient@123</Text>
              </View>
            </View>
          ) : null}
        </ScrollView>
