...
const FamilyRow = React.memo(function FamilyRow({
  ...
  return (
    <Card style={{ marginBottom: spacing.md }} testID={`family-card-${m.id}`}>
...
export default function FamilyList() {
  ...
  const onEdit = useCallback((m: FamilyMember) => {
    router.push({ pathname: "/family/add", params: { id: m.id } });
  }, [router]);

  const onRemove = useCallback((m: FamilyMember) => {
  ...
  const renderItem = useCallback(({ item }: { item: FamilyMember }) => (
    <FamilyRow m={item} onEdit={onEdit} onRemove={onRemove} />
  ), [onEdit, onRemove]);
