...
const FaqRow = React.memo(function FaqRow({ f, index, isOpen, onToggle }: { f: Faq; index: number; isOpen: boolean; onToggle: (i: number) => void }) {
  return (
    <TouchableOpacity activeOpacity={0.8} onPress={() => onToggle(index)} style={styles.faqRow} testID={`faq-item-${index}`}>
...
export default function HelpSupport() {
  ...
  const load = useCallback(async () => {
    try { const f = await api.get<{ faqs: Faq[] }>("/support/faqs", false); setFaqs(f.faqs); } catch {}
    try { const t = await api.get<Ticket[]>("/support/tickets"); setTickets(t); } catch {}
  }, []);
  ...
  const renderTicket = useCallback(({ item }: { item: Ticket }) => <TicketCard t={item} />, []);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bgApp }} edges={["top"]}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabsRow}>
        {(["faqs", "contact", "tickets"] as const).map(k => (
          <TouchableOpacity key={k} onPress={() => setTab(k)} style={[styles.tab, tab === k && styles.tabActive]} testID={`help-tab-${k}`}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        {tab === "faqs" ? (
          <FlatList data={faqs} keyExtractor={(_, i) => `faq-${i}`} renderItem={renderFaq} ListFooterComponent={faqFooter}
        ...
        {tab === "contact" ? (
          <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: 40 }} keyboardShouldPersistTaps="handled"
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 4 }}>
              {CATEGORIES.map(c => (
                <TouchableOpacity key={c.key} onPress={() => setCategory(c.key)} style={[styles.chip, category === c.key && styles.chipActive]} testID={`help-cat-${c.key}`}>
            ...
            <Input testID="help-subject-input" label="Subject" icon="chatbubble-outline" value={subject} onChangeText={setSubject} placeholder="Brief summary" />
            <Input testID="help-message-input" label="Describe your issue" icon="document-text-outline" value={message} onChangeText={setMessage} placeholder="Please share details so we can help you faster" multiline style={{ minHeight: 120, textAlignVertical: "top", paddingTop: 12 }} />
        ...
        {tab === "tickets" ? (
          <FlatList data={tickets} keyExtractor={(item) => item.id} renderItem={renderTicket} ListEmptyComponent={renderTicketsEmpty}
